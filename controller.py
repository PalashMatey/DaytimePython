...
def exercise():
    ...
    global curuser_speed, curuser_dist, curuser_hr
    global state
    ...
    while True:
        if (state==7):
            ...
            curuser_hour = time_elapsed/3600
            curuser_min = (time_elapsed%3600)/60
            curuser_sec = (time_elapsed%3600)%60
            curuser_dist = curuser_dist + curuser_speed / 3600.0
            if (hr_measured - curuser_hr < 100.0 and curuser_hr - hr_measured < 100.0):
                curuser_hr = hr_measured
            #update local database
            time.sleep(1)

def show_lcd():
    global lcd
    global isnewuser, newuser_id, newuser_age, newuser_mf, newuser_weight
    global curuser_id, curuser_hr_target, curuser_target_zone
    global curuser_hour, curuser_min, curuser_sec
    global curuser_speed, curuser_dist, curuser_hr
    global state
...
t3 = Thread(target=exercise,args=()) 
t1.start()
t2.start()
t3.start()
t1.join()
